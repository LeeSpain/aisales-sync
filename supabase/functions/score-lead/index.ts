...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      // AI call failed — return a safe default so the pipeline continues
    ...
    // Outer catch: only for request parsing / rate limit / dead switch errors
    console.error("score-lead error:", e);
    // Still return a usable response so the pipeline never crashes
