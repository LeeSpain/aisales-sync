...
function buildQueries(
  ...
  // Target market specific queries (highest priority — these are what the user actually wants)
  for (const market of targetMarkets.slice(0, 3)) {
    queries.push(`${market} ${geographicFocus}`);
  }

  // Industry + location
  if (industries.length > 0) {
    queries.push(`${industries.join(" OR ")} businesses ${geographicFocus}`);
  }

  // Custom keywords
  for (const kw of keywords.slice(0, 2)) {
    queries.push(`${kw} ${geographicFocus}`);
  }
  ...
  try {
    ...
    if (allLeads.length < 20) {
      for (const query of queries) {
        const results = await serperSearch(serperKey, query, 15);
    ...
    return jsonResponse({ leads: allLeads.slice(0, 50), count: Math.min(allLeads.length, 50), source: "google_maps" });
  } catch (e) {
