...
CREATE TABLE public.companies (
  ...
  fleet_or_inventory JSONB,
  target_markets JSONB DEFAULT '[]'::jsonb,
  geographic_range TEXT,
  pricing_summary TEXT,
  tone_preference TEXT DEFAULT 'professional' CHECK (tone_preference IN ('formal','professional','casual','friendly')),
  industry TEXT,
  ai_profile JSONB,
...
-- Add foreign key from companies.owner_id to profiles
ALTER TABLE public.companies ADD CONSTRAINT fk_companies_owner FOREIGN KEY (owner_id) REFERENCES public.profiles(id) ON DELETE SET NULL;
...
CREATE POLICY "Users can update own company" ON public.companies FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE profiles.id = auth.uid() AND profiles.company_id = companies.id)
);
CREATE POLICY "Users can insert company" ON public.companies FOR INSERT TO authenticated WITH CHECK (true);
